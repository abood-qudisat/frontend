...
const CourseCard = ({ course }) => {
    return (
        ...
        >
            {/* Course Image */}
            <div className="h-40 w-full bg-cover bg-center" style={{ backgroundImage: `url(${course.image})` }} />

            {/* Course Content */}
            <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                    <h2 className="text-lg font-semibold text-gray-800">{course.title}</h2>
                    <span className="text-emerald-600 font-bold text-sm">${course.price}</span>
                </div>
                ...
                {/* Description */}
                <p className="text-sm text-gray-600 mb-3 text-left">{course.description}</p>
